...
pdf(file.path(path_plots, "Read_quality_profile_aggregated.pdf"))
...
pdf(file.path(path_plots, "Error_rates_learning.pdf"))
...
# Export files specifically for Phyloseq:


## as text files
# We need to format the data a little bit.
...
cat(paste0(">", names(asv_seq), "\n", asv_seq),
    sep = "\n",
    file = file.path(path_results, "asv.fasta"))
